...
// Every documented command has to reach a handler. The set used to live in
// three places -- the usage text, the early switch in main, and the switch in
// run -- with nothing tying them together, so a command added to one and
// forgotten in another was silent: listed in the help and answered with
// "unknown command", or reachable and undocumented.
//
...
func TestEveryDocumentedCommandIsWiredUp(t *testing.T) {
	for _, c := range commands {
		if c.early {
			// Handled before run, through controlService. That it is in the
			// table at all is what keeps it out of run's default branch.
			continue
		}
		t.Run(c.name, func(t *testing.T) {
			verb := c.verb()
			err := run(verb, "/nonexistent/smtprelayd.toml", false, "", false, 0, "read")
			if err == nil {
				return // did its work without needing the configuration
			}
			if strings.Contains(err.Error(), "unknown command") {
				t.Errorf("%q is in the usage text but falls through to run's default branch", verb)
			}
			if strings.Contains(err.Error(), "not wired up") {
				t.Errorf("%q is in the command table but has no case in run", verb)
			}
		})
	}
}
...
func TestAnUnknownCommandIsRefused(t *testing.T) {
	err := run("definitely-not-a-command", "/nonexistent/smtprelayd.toml", false, "", false, 0, "read")
	if err == nil {
